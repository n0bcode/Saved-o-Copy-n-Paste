...
HTMLElement.prototype.click = function (...args) {
  ...
  } else return oriClick.apply(this, arguments);
};

// Detect and override input elements that uses .showPicker()
var oriShowPicker = HTMLInputElement.prototype.showPicker;
HTMLInputElement.prototype.showPicker = function () {
  if (this.matches("input[type='file']")) this.click();
  else return oriShowPicker.apply(this, arguments);
};
...
// Save file to recent files list (global function to be used across scripts)
function saveToRecentFiles(file) {
  // Check if recentFiles is defined (from content.js)
  if (typeof recentFiles !== "undefined" && typeof maxRecentFiles !== "undefined") {
    ...
    try {
      ...
      logging("Saved file to recent files: " + (file.name || "Unnamed File"));
    } catch (error) {
      logging("Error saving recent files to localStorage: " + error);
    }
  } else {
    logging("recentFiles or maxRecentFiles not defined, cannot save to recent files.");
  }
}

// Update logging to be accessible globally
...
function previewImage(webCopiedImgSrc, readerEvent, blob, requestedOverlayID) {
  if (overlayID !== requestedOverlayID) return;
  ...
  if (!imagePreview) {
    ...
    if (blob.type.split("/")[0] == "image") {
      ...
      imagePreview.onload = () => {
        ...
        if (
          (imagePreview.naturalWidth < 272 && imagePreview.naturalHeight < 153) ||
          (imagePreview.naturalHeight < 153 && imagePreview.naturalWidth <= imagePreview.naturalHeight)
        ) {
    ...
    // Preview PDF type
      ...
      currentObjectURL =
        window.URL.createObjectURL(new Blob([readerEvent.target.result], { type: blob.type })) +
        "#scrollbar=0&view=FitH,top&page=1&toolbar=0&statusbar=0&navpanes=0";
      imagePreview.src = currentObjectURL;
      try {
        imagePreviewContainer.appendChild(imagePreview);
      } catch (error) {
    ...
    // Preview video types
      ...
      imagePreview.onloadedmetadata = () => {
        if (imagePreview.videoWidth == 0 || imagePreview.videoHeight == 0) previewGenericFile("audio_file");
        else
          try {
            imagePreviewContainer.appendChild(imagePreview);
          } catch (error) {
    ...
    // Preview audio type
    else if (blob.type.split("/")[0] == "audio") previewGenericFile("audio_file");
    else if (blob.type.split("/")[0] == "text") previewGenericFile("text_file");
    // Preview other file types
    else previewGenericFile("blank_file");
  }
  ...
  function previewGenericFile(fileTypeIcon) {
          ...
          window.top.postMessage(
            {
              ...
              iframe: document.head
                ...
                .getAttribute("id"),
              Path: `media/${fileTypeIcon}.webp`,
            },
            "*"
          );
        else window.top.postMessage({ Type: "getURL", Path: `media/${fileTypeIcon}.webp` }, "*");

        window.onmessage = (event) => {
          if (event.data.Type == "getURL-response") imagePreview.src = event.data.URL;
        };
      } catch (error) {
        logging(error);
      }
    ...
    if (blob.name.length > 25) {
      ...
      while (baseName[baseName.length - 1] === " ") baseName = baseName.slice(0, -1);
      baseName += "..";
    }
...
function createOverlay(event) {
  ...
  const hexArray = Array.from(crypto.getRandomValues(new Uint8Array(16))).map((byte) =>
    byte.toString(16).padStart(2, "0")
  );
  const uuid = [
    hexArray.slice(0, 4).join(""),
    hexArray.slice(4, 6).join(""),
    "4" + hexArray.slice(6, 7).join(""),
    ((parseInt(hexArray[8], 16) & 0x3) | 0x8).toString(16) + hexArray.slice(9, 11).join(""),
    hexArray.slice(11, 16).join(""),
  ].join("-");
    ...
    return new Promise((resolve) => {
      ...
      if (document.head.querySelector("script[overlayhtml]")) {
        ...
        try {
          ...
          else window.top.postMessage({ Type: "getURL", Path: "overlay.html" }, "*");
          window.onmessage = (event) => {
      ...
      if (urlToFetch) {
        fetch(urlToFetch)
          ...
          .then((html) => {
            ...
            if (document.querySelector(".cnp-overlay")) return;

            // Checks browser type to either use trustedTypes or not (Google Slides requirement)
            if (isFirefox) overlay.innerHTML = html;
            else
              overlay.innerHTML = trustedTypes
                .createPolicy("forceInner", { createHTML: (to_escape) => to_escape })
                .createHTML(html);
            ...
            if (tooMuchRight >= innerWidth) overlayLeftPos -= overlayContent.offsetWidth;
            if (tooMuchBottom >= innerHeight) overlayBottomPos -= overlayContent.offsetHeight;
            ...
            if (overlayFileInput.multiple) overlay.querySelector("#cnp-upload-text").textContent = "Upload Files";
            ...
            overlayFileInput.onchange = (event) => {
              ...
              [...originalInput.files, ...event.target.files].forEach((file) => {
                fileList.items.add(file);
                // Save to recent files
                saveToRecentFiles(file);
              });
            ...
            overlay.ondragleave = (event) => {
              if (!overlay.contains(event.relatedTarget)) CNP_dropText.style.display = "none";
            };
            ...
            overlay.ondrop = (event) => {
              ...
              const fileList = new DataTransfer();
              // Reattach previous files and append new ones
              const excludedFolders = [...event.dataTransfer.files].filter(
                (file) => !(file.size === 0 && file.type === "")
              );
              [...originalInput.files, ...excludedFolders].forEach((file) => {
                fileList.items.add(file);
                // Save to recent files
                saveToRecentFiles(file);
              });
              ...
              async (event) => {
                ...
                if (overlayFileInput) {
                  ...
                  if (dataTransfer.files.length > 0) {
                    ...
                    const excludedFolders = [...dataTransfer.files].filter(
                      (file) => !(file.size === 0 && file.type === "")
                    );
                    if (excludedFolders.length == 0) {
                      ...
                      const readPromises = [...dataTransfer.files]
                        ...
                        .map((file) => {
                          ...
                          if (parseInt(badge.innerText) > 1) badge.style.display = "inline-block";
                    ...
                    if (statusMap.get("fail") >= 1 && statusMap.get("success") <= 0) noImage();
                    else {
                      const imagePreviewContainer = document.querySelector("#cnp-preview-container");
            ...
            if (!isPasteListenerTriggered) window.top.postMessage({ Type: "paste" }, "*");
...
// Console logging for errors and messages (already defined above, but kept for compatibility)
function logging(message) {
  console.log("%c📋 Copy-n-Paste:\n", "font-weight: bold; font-size: 1.3em;", message);
  window.top.postMessage(("%c📋 Copy-n-Paste:\n", "font-weight: bold; font-size: 1.3em;", message), "*");
}
